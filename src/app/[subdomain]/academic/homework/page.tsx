'use client'
import { HomeworkForm } from '@/Schemas/Schema'
...
import HomeworkTable from '@/DataTables/Academic/HomeworkTable'
...
const Homework = () => {
    ...
    } = useForm<HomeworkParams>({
        resolver: zodResolver(HomeworkForm, {}, { raw: true }),
    })

    const router = useRouter()
    const { ToastError, ToastSuccess } = useToast()

    const onSubmit: SubmitHandler<HomeworkParams> = async (
        data: HomeworkParams,
    ) => {
        // console.log(data)
                    ...
                    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                        <div className="border-b border-stroke px-6.5 py-3 dark:border-strokedark  flex justify-between">
                            ...
                            <div className="flex justify-center rounded bg-primary p-3 font-medium text-gray hover:bg-opacity-90">
                                <Link href="homework/add">Add Homework</Link>
                            </div>
                        </div>
                        <HomeworkTable />
                    </div>
