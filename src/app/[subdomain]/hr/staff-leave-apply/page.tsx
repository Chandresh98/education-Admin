'use client'
import { StaffLeaveApplyForm } from '@/Schemas/Schema'
...
import StaffLeaveApplyTable from '@/DataTables/HR/StaffLeaveApplyTable'
import Select from '@/components/Forms/Select'
import Link from 'next/link'

const StaffLeaveApply = () => {
    const {
        register,
        handleSubmit,
        formState: { errors, isSubmitting },
    } = useForm<StaffLeaveApplyParams>({
        resolver: zodResolver(StaffLeaveApplyForm, {}, { raw: true }),
    })

    const router = useRouter()
    const { ToastError, ToastSuccess } = useToast()

    const onSubmit: SubmitHandler<StaffLeaveApplyParams> = async (
        data: StaffLeaveApplyParams,
    ) => {
        // console.log(data)
                    ...
                    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
                        <div className="border-b border-stroke px-6.5 py-3 dark:border-strokedark  flex justify-between">
                            ...
                            <div className="flex justify-center rounded bg-primary p-3 font-medium text-gray hover:bg-opacity-90">
                                <Link href="staff-leave-apply/add">
                                    Add Staff Leave Apply
                                </Link>
                            </div>
                        </div>
                        <StaffLeaveApplyTable />
                    </div>
                </div>
            </div>
        </DefaultLayout>
