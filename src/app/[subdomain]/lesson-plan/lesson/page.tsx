...
import { LessonForm } from '@/Schemas/Schema'
...
import LessonTable from '@/DataTables/Lesson-Plan/LessonTable'
...
const Lesson = () => {
    ...
    } = useForm<LessonParams>({
        resolver: zodResolver(LessonForm, {}, { raw: true }),
    })

    const router = useRouter()
    const { ToastError, ToastSuccess } = useToast()

    const onSubmit: SubmitHandler<LessonParams> = async (
        data: LessonParams,
    ) => {
        // console.log(data)
    ...
    const [lessonComponent, setLessonComponents] = useState<number[]>([0])
    const addMore = () => {
        setLessonComponents([...lessonComponent, lessonComponent.length + 1])
    }
    const onDelete = (id: number) => {
        setLessonComponents(lessonComponent.filter(cid => cid != id))
    }
    return (
        <DefaultLayout>
